...
## Update — the function catalog, and the two efficiency papers

The operation vocabulary this document describes is no longer a union the builders switch over. It
is a *catalog* (`src/editor/functions/catalog.ts`) that each part of the app reads: the ribbon
applies from it, the admin form builds its fields from it, `parseOperations` validates against it,
and both the worked answer and the answer key are derived from the same entry. `sdd/editor-functions.md`
is the design; `.claude/skills/editor-functions/SKILL.md` is how to add one.

What changed here specifically:

- **A question names its text instead of measuring it.** `WordScope` is now a `SelectionSpec` — the
  third word, the second sentence, the fourth paragraph, a phrase, a phrase wherever it appears —
  resolved when the paper is marked rather than at authoring time, so one key stays correct in both
  languages. `'all'` and a bare character range still mean what they always did.
- **A question can have several steps, and can start formatted.** `steps` is a list of
  (selection, operations); `initial` is the formatting the passage arrives with, which is what makes
  "remove the highlight" a question rather than a no-op. Both live in `content` on the row.
- **Two papers ship as a migration.** `db/migrations/0016_word_efficiency_papers.sql` loads Word
  Efficiency Tests 4 and 5, published, attached to the oldest exam (creating one on a fresh
  database). It is generated by `npx tsx scripts/generateWordEfficiencyMigration.ts` from
  `src/exam/papers/wordEfficiency.ts` rather than written by hand, because the TypeScript is what
  the tests mark: `wordEfficiency.test.ts` marks every question's worked answer against its own
  derived key in both languages, and `wordEfficiency.migration.test.ts` pulls the rows back out of
  the shipped SQL, reads them with `draftFromRow` — the same function the player uses — and marks
  those. The two together cover the whole path from a paper to a sitting without needing a database.
- **Questions the model cannot mark were replaced, not faked.** The source lists ask for table
  operations, headers, page borders, watermarks, page numbers and gutter margins; none of those are
  part of the document the marker sees. Each is replaced by a paragraph question exercising the same
  ribbon group, and says in a comment what it stands in for.
