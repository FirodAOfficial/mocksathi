...
export interface RunFormatting {
  ...
  highlight?: string;
  /**
   * Word's Emboss / Engrave character effects (Font dialog -> Effects).
   * Mutually exclusive: text is either raised or carved, never both.
   */
  effect?: 'emboss' | 'engrave';
  /** Character scale as a percentage of normal width, e.g. 200. */
  charScale?: number;
  /**
   * Character spacing in points: positive is Expanded, negative Condensed.
   * Word expresses these as two controls; one signed number says the same thing.
   */
  charSpacing?: number;
}
...
export interface TableCell {
  /** A header cell renders bold and centred, as Word's header row does. */
  header: boolean;
  /** Cells hold block content, so a cell can carry a list or several lines. */
...
/** Images and page breaks become further members without disturbing consumers. */
export type DocumentBlock = ParagraphBlock | TableBlock;

export function isParagraph(block: DocumentBlock): block is ParagraphBlock {
  return block.type === 'paragraph';
}

export function isTable(block: DocumentBlock): block is TableBlock {
  return block.type === 'table';
}

/** Every paragraph in a block, descending into table cells. */
export function paragraphsOf(block: DocumentBlock): ParagraphBlock[] {
...
export function isDocumentEmpty(model: DocumentModel): boolean {
  return model.body
    ...
    .every((paragraph) => paragraph.runs.every((run) => run.text.trim() === ''));
}
