...
type RawMock = Omit<MockSummary, 'mockType' | 'questionCount'>;

/**
 * Fills in `mockType` and `questionCount`, derived rather than hand-authored
 * per row: `mockType` alternates word/excel by mock number (mixed for the two
 * "Full Revision" papers), and `questionCount` mirrors the "· 100 Qs" already
 * embedded in every `paperName` below.
 */
function withMockMeta(mock: RawMock): MockSummary {
  const mockType: MockSummary['mockType'] = mock.paperName.startsWith('Full Revision')
    ? 'mixed'
    : mock.mockNumber % 2 === 0
      ? 'excel'
      : 'word';
  return { ...mock, mockType, questionCount: 100 };
}
...
const RAW_RECENT_MOCKS: RawMock[] = [
  {
    mockNumber: 18,
