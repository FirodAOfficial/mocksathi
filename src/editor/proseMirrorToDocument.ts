 ...
 * asserting round-trip fidelity, and as the input the marking engine flattens.
 */
...
function readTable(node: JSONContent): TableBlock {
  return {
    type: 'table',
    rows: (node.content ?? [])
      ...
      .map((row) => ({
        cells: (row.content ?? [])
          ...
          .map((cell) => ({
            ...
            // Cells hold block content; a nested table would be dropped, which
            // is reported by the parser rather than silently flattened.
            blocks: readBlocks(cell.content ?? [], null).filter(
  ...
  for (const mark of marks) {
    switch (mark.type) {
      ...
      case 'textStyle': {
        ...
        // Font-dialog formatting rides on the same mark.
        if (attrs.effect === 'emboss' || attrs.effect === 'engrave') formatting.effect = attrs.effect;
        if (typeof attrs.charScale === 'number' && Number.isFinite(attrs.charScale)) {
          formatting.charScale = attrs.charScale;
        }
