...
function buildBlocks(blocks: DocumentBlock[]): JSONContent[] {
  ...
  for (const block of blocks) {
    if (!isParagraph(block)) {
      // A table is never part of a list run, so any open list closes first.
      while (stack.length > 0) closeTop();
      output.push(tableToNode(block));
      continue;
    }
...
function tableToNode(table: TableBlock): JSONContent {
  return {
    type: 'table',
    content: table.rows.map((row) => ({
      ...
      content: row.cells.map((cell) => ({
        ...
        // A cell holds block content, so its paragraphs go through the same
        // builder as the body — a cell can contain a list like anything else.
        content: buildBlocks(cell.blocks),
      })),
    })),
  };
}
