...
export function useQuestionAnswers(
  ...
  const persist = useCallback(
    (instance: Editor, number: number): void => {
      ...
      const untouched = instance.schema.nodeFromJSON(defaultAnswerDocument(question)).toJSON();

      if (documentsEqual(current, untouched)) clearAnswer(number);
      else saveAnswer(number, current);
    },
    [attempt, saveAnswer, clearAnswer],
  );
  ...
  useEffect(() => {
    ...
    if (previous === null && adoptInitialContent) {
      ...
      install(editor, stored ?? defaultAnswerDocument(question));
    }

    installedRef.current = selectedNumber;
  }, [editor, enabled, selectedNumber, attempt, adoptInitialContent, install, persist]);
  ...
  const clearCurrent = useCallback(() => {
    ...
    install(editor, defaultAnswerDocument(question));
    editor.commands.focus('start');
  }, [editor, attempt, selectedNumber, clearAnswer, install]);
